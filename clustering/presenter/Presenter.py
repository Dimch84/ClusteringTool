...
class Presenter:
    def __init__(self):
        ...
        self.viewer.load_from_model(self.model)
        self.scores = scores
        self.viewer.show()
    ...
    def add_algo_run_pushed(self):
        ...
        result = self.viewer.show_add_algo_run_dialog(algo_run_attr=algo_run_dialog_attr)
        if result is None:
            return
        ...
        self.viewer.add_algo_run(AlgoRun(
    ...
    def remove_algo_run(self, algo_run_attr):
        algo_run = next(algo_run for algo_run in self.model.algo_runs
                        ...
                        set(algo_run.algo_run_attrs.scores) == set(algo_run_attr.calculated_scores.keys())
        )
        self.model.remove_algo_run(algo_run)
        self.viewer.remove_algo_run(algo_run_attr)

    def add_algo_pushed(self):
        file = self.viewer.show_open_file_dialog(
    ...
    def add_dataset_pushed(self):
        ...
        result = self.viewer.show_add_dataset_dialog(df.columns.tolist())
        ...
        self.viewer.add_dataset(dataset.name)
    ...
    def change_cur_dataset(self, dataset_name):
        ...
        self.viewer.change_cur_dataset(dataset_name)

    def save_session_pushed(self):
        file = self.viewer.show_save_file_dialog(
